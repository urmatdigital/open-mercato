 ...
 * stack, run count, and last-activity. The payload carries no workflowInstanceId, so the
 * recompute resolves it from the run row (same module — no cross-module read).
 */
export const metadata = {
  event: 'agent_orchestrator.run.completed',
  persistent: true,
  id: 'agent_orchestrator:process-run-completed',
}
...
export default async function handle(
  ...
  await recomputeFromEvent(em, record, { resolveInstanceIdFromRunId: runId })
}
