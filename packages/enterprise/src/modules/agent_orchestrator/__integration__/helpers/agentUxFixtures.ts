 ...
 *
 * `process_instances` is a projection maintained by event subscribers — there is
 * deliberately no create API for it, so execution-detail UI assertions seed the
 * row directly (same dbFixtures precedent as agentPerfFixtures.ts, incl. its
...
/** Inserts one `process_instances` projection row; returns its workflow instance id. */
...
export async function deleteAgentProcessesForOrganization(organizationId: string | null): Promise<void> {
  ...
  await withClient(async (client) => {
    await client.query('delete from process_instances where organization_id = $1', [organizationId])
  })
}
