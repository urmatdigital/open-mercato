...
import type {
  ApproveEvalCaseCommandInput,
  ApproveEvalCaseCommandResult,
} from '../../../../commands/corrections'

/**
 * Engineer approval of a drafted eval case (draft → approved). Editable target,
...
export async function POST(req: Request, ctx: RouteContext) {
  ...
  try {
    const { result } = await commandBus.execute<ApproveEvalCaseCommandInput, ApproveEvalCaseCommandResult>(
      'agent_orchestrator.evalCases.approve',
      {
        input: {
...
export const openApi: OpenApiRouteDoc = {
  ...
  summary: 'Approve an eval case',
  methods: {
    POST: {
      summary: 'Approve a drafted eval case so it joins the exported regression set',
      description:
      ...
      responses: [{ status: 200, description: 'The approved eval case', schema: resultSchema }],
      errors: [
