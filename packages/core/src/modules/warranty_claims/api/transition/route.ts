...
import { transitionClaimInputSchema, type TransitionClaimInput } from '../../data/validators'
...
function toTransitionInput(scopedPayload: Record<string, unknown>): TransitionClaimInput {
  return transitionClaimInputSchema.parse({
    id: scopedPayload.id,
    toStatus: scopedPayload.toStatus,
    rejectionReasonCode: scopedPayload.rejectionReasonCode,
    resolutionSummary: scopedPayload.resolutionSummary,
  })
}
...
export async function POST(req: Request) {
  try {
    ...
    const input = toTransitionInput(scopedPayload)
    const guarded = await runGuard(req, context, input)
    if (!guarded.ok) {
      return guarded.response
    }
    const commandInput = guarded.modifiedPayload ? toTransitionInput(guarded.modifiedPayload) : input
    ...
    logger.error('warranty_claims.transition.post failed', { err })
...
export const openApi: OpenApiRouteDoc = {
  tag: 'Warranty Claims',
  summary: 'Transition warranty claim',
  methods: {
    POST: {
      summary: 'Move a claim through the lifecycle',
      requestBody: { contentType: 'application/json', schema: transitionClaimInputSchema },
      responses: [
        {
          status: 200,
          description: 'Claim transitioned',
