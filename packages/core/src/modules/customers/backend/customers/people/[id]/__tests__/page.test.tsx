...
// Regression coverage for issue #6158: opening the person detail page with a
...
jest.mock('next/navigation', () => ({
  ...
  usePathname: () => '/backend/customers/people/test',
}))
...
jest.mock('../../../../../components/detail/PersonHighlights', () => ({ PersonHighlights: () => null }))
...
jest.mock('../../../../../components/detail/InlineEditors', () => ({
  renderLinkedInDisplay: () => null,
  renderTwitterDisplay: () => null,
  renderMultilineMarkdownDisplay: () => null,
  InlineDictionaryEditor: () => null,
}))

describe('CustomerPersonDetailPage — malformed id handling (#6158)', () => {
  beforeEach(() => {
  ...
  it('shows the localized not-found state — not the raw server error text — when the id is malformed (400)', async () => {
    ...
    renderWithProviders(<PersonDetailPage params={{ id: 'not-a-valid-uuid' }} />)

    await waitFor(() => {
      expect(screen.getByTestId('record-not-found')).toBeInTheDocument()
    })
    expect(screen.getByTestId('record-not-found')).toHaveTextContent('Person not found')
  ...
  it('still shows the not-found state for a well-formed but missing id (404)', async () => {
    ...
    renderWithProviders(<PersonDetailPage params={{ id: '2408107d-0000-4000-8000-000000000099' }} />)
