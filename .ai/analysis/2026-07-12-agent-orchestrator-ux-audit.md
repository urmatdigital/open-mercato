...
| P0-5 | **Overview renders fabricated numbers as fact.** "Where humans stepped in" shows hardcoded 412/188/96/61/53 interventions with **no Sample label**, next to real KPIs, and deep-links to the real audit page whose data won't match. Sister issue: a hardcoded domain chip for every tenant. | `backend/overview/page.tsx:238-246,400-433,260`; live `65-overview-populated.png` | Add the Sample/pending marker (pattern exists on the same page) or hide the section; derive/remove the domain chip. |
...
A domain-specific chip (Overview), a domain-specific search placeholder + subject column (Caseload), domain-specific subtitle and party labels (Processes), and a subject column with `<domain>Id/dealId` key-sniffing (Audit, Overview stuck queue, Agents detail) — while the shipped example agents are **deals**. *Fix: neutral vocabulary ("Subject", "Reference") driven by `subjectType`/declared facts; one sweep across 5 pages.*
