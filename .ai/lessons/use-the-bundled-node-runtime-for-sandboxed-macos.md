...
**Rule**: When harness tests fail broadly with `dyld` or blocked Node-library errors, put the workspace's self-contained Node runtime first on `PATH` and rerun the same complete gate command. A full `yarn test` run can reach nested create-app suites before exposing the blocked Homebrew dependency, so a targeted package rerun is not equivalent evidence. Do not diagnose the resulting cascade as product failures until the runtime can start inside the sandbox. If the bundled-runtime rerun ends with a one-off Jest worker `SIGSEGV` after its assertions pass, rerun that test file in-band before retrying the full gate; do not change unrelated product code unless the failure reproduces. Build the docs search index first when a fresh checkout does not contain it.
