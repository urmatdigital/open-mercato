...
5. **Block status has no machine trace.** A composed screen mixes proposed new UI with stock platform surfaces, yet nothing distinguishes them: whether a block is a proposal, already shipped, or default behavior lives only in people's heads. Drafting, review, and any later promotion to code all depend on that distinction being explicit in the document.
...
- **Renderer** — `/backend/design-system/mockups` (list + per-status counts) and `/backend/design-system/mockups/[slug]` (the live render). Blocks render the real components from the gallery registry inside the real backend shell — real tokens, real dark mode. A toolbar toggle switches the **annotation layer** on/off. The language is review-margin, not markup-on-content: an annotated block gets a slim status accent on its leading edge (the sanctioned `border-l-4 border-status-*-border` pattern), and every annotation lives as a dot-label entry (6px radius, no pills) in a **side ledger** — hovering a ledger entry emphasizes the paired block's accent and vice versa, like a review comment beside a diff line. `userStory` and notes render in the ledger entry, never on the screen. Layer off yields a clean render for screenshots.
...
- **Counts, not reports** — the ledger header and the list page show per-status block counts (implemented / proposed / om-default / placeholder). Statuses are working mechanics for drafting, review, and promotion — deliberately not a client-reporting layer; richer per-user-story reporting is out of scope (future work). A screenshot script reuses the QA ephemeral-env conventions to export clean and annotated PNGs for spec documents.
...
- **UX skills, first pair** — `om-ux-heuristics` critiques a mockup against an encoded checklist (Nielsen's 10 + the project's executable UX contracts) and writes findings into the document as a new `finding` annotation type (severity + heuristic id), rendered in the same margin-rail-and-ledger language (severity-toned rail segments and ledger entries; nothing drawn on content). `om-ux-copy` runs a microcopy pass over every text-bearing prop and emits ready i18n keys for en/pl/es/de as a companion copy file.
...
- **Renderer page:** loads and validates the document server-side (a failing document renders the standard `ErrorNotice` with the zod issues — never a blank page), then renders the layout tree with real components inside a bordered stage sized by the `width` preset. Toolbar: overlay toggle (`SegmentedControl`: Clean / Annotated), user-story filter (dims non-matching blocks), auto-refresh poll (2s; the dev server re-reads per request, so edit-to-screen latency is one poll tick), and the ledger with its per-status counts.
...
- **Findings layer** (Phase 2) — the same margin-rail-and-ledger language as statuses, so there is one annotation system, not two: a finding adds a severity-toned segment to the block's margin rail (`critical`/`high` → `status-error` tokens, `medium` → `status-info`, `low` → `status-neutral`) and a numbered entry in the ledger's findings section; selecting the entry scrolls to the block and widens its rail accent — content itself is never outlined, boxed, or dimmed by a finding. Findings whose `atHash` no longer matches the current document hash render dimmed in the ledger with a "stale" label — a critique of a screen that has since changed is flagged, not silently trusted.
...
`yarn ds:mockups:snapshot <slug> <label>` (also a studio action) copies the current document to `versions/<slug>@<label>.mockup.json` — snapshots are ordinary schema-valid files, covered by the same integrity test. The diff view (`/backend/design-system/mockups/[slug]?compare=<label>` or two labels) renders both versions side by side and computes a block-level delta by id, expressed in the same rail-and-ledger language: **added** blocks carry a `status-success` rail, **removed** blocks render as ghost placeholders with a `status-error` rail, **changed** blocks (entry, variant, props, or annotation) a `status-info` rail, moved-only blocks `status-neutral` — with the full delta listed in the ledger. Same vocabulary as the rest of the composer; no new visual language. The delta is also available as JSON (`GET .../diff`) for changelog notes.
...
type MockupCoverage = {
  ...
  userStories: string[]   // tags present in the document; per-story reporting deliberately out of scope (future work)
...
1. **Phase 1 — Core** (ship order: schema + integrity check → renderer + annotation layer → ledger counts + GET/annotation-PUT routes → `om-ds-mockup` skill + screenshot script + `om-spec-writing`/`om-backend-ui-design` sections). Golden mockup committed with the first step. Exit gate: Phase 1 validation section green, guardian REVIEW zero findings.
...
- **Ledger counts** — per-status totals correct for the golden mockup; placeholders counted separately.
...
| # | Path | Assertions |
|---|---|---|
| 1 | List + counts | `design_system.view` user sees golden mockup row with correct per-status counts and user stories; empty state renders when filtered to nothing. |
...
| 3 | Story filter + API parity | Story filter dims non-matching blocks; ledger counts match the GET API for the same slug. |
...
- **Integration:** (6) studio round-trip — enter edit mode, swap a block's variant, edit a prop via the generated form, reorder, save; file on disk reflects the change; a concurrent out-of-band file edit then a second save yields the 409 reload prompt. (7) share link — minted URL opens the public page logged-out; overlay toggles; watermark present; expired token → 404 page; no authenticated chrome in DOM. (8) diff view — `@v1` vs `@v2` renders side by side with the expected added/removed/changed rail tones and ledger delta. (9) findings layer — findings-bearing fixture shows severity rail segments and ledger entries with no markup on block content; stale finding renders dimmed in the ledger with label.
...
| 4 | **Stale statuses lie** — a `proposed` block ships but nobody flips the annotation. | Medium | Review accuracy | Status flip is a named `om-spec-writing` step and a one-click studio action; last-modified visible; promote closes the loop for scaffolded screens. Process guarantee, not mechanical. | Medium |
