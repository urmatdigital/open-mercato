...
4. **No cross-module self-service launch.** A hackathon-quality precedent developed outside this repository already proves the shape is wanted: a module-local sandbox page lets a user pick a workflow and bulk-start instances against selected rows — but it is module-locked and does not exist on the branch this spec targets. The underlying need (launch a preconfigured agent/workflow task against a selected business record from any module's list) has no generic home.
