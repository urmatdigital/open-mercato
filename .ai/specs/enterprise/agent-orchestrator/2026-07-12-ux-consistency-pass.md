...
| Domain-specific vocabulary on a generic platform | `overview.domain`, `caseload.searchPlaceholder`, `caseload.col.*`, `audit.col.*`, `agentDetail.recent.*`, `process.*`/`list.col.*`/`list.subtitle` (`i18n/en.json:44,109,132,171,261,366-383`); subject-key sniffing in `audit/page.tsx:123`, `agents/[id]/page.tsx:165`, `caseload/page.tsx:289`, `overview/page.tsx:221` |
...
| Key | Old | New |
|---|---|---|
| `overview.domain` | a domain-specific adjudication label | **already removed by spec 1** (chip + key ×4) — this spec **verifies only** (grep for residual references) |
| `caseload.searchPlaceholder` | a domain-specific search placeholder | "Search subjects, agents…" |
| `caseload.col.claim`, `audit.col.claim`, `agentDetail.recent.claim`, `process.list.col.claim` | "Claim" | "Subject" |
| `process.factOwner` | domain-specific party label | "Owner" |
...
- **2026-07-12**: **Implemented** (all six phases, one commit each on `feat/agent-orchestrator-mvp`):
  ...
  - Phase 4 `adbd10584` — guardrail/tool enum label maps ×4, `runtime.native` key ×4, Q5 sweep (8 en swaps + de/es/pl domain-term neutralization), `components/subjectRef.ts` (4 call sites), PL „decyzja" unification + „Skrzynka" + real `guardrailFlagged` translation.
