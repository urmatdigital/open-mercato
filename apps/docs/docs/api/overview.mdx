...
- **Exporting:** Add `format=csv|json|xml|markdown` to a list endpoint to download the result set as a file instead of a JSON page. See the [Exporting data guide](./exporting).
...
- Both export routes stay publicly reachable, but the access-control metadata of each operation (the `Requires features/roles` description lines and the `x-require-features` / `x-require-roles` extensions) is served only to signed-in staff callers. Anonymous clients receive the same document with those internal identifiers stripped, so the ACL feature names of your deployment are never published.
