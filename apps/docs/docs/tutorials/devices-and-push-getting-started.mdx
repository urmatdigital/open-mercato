...
### Per-type channel eligibility (business types ship without push)
Notification types can declare which channels they may deliver on. **Every user-configurable
built-in type does so explicitly:** business and actionable types ship
`channels: ['in_app', 'email']`, while the `demo.silent_ping` and `demo.push_playground` fixtures
ship `channels: ['push']`. Push is therefore off for ordinary business notifications out of the
box, so connecting a push provider does not blast the entire catalogue to phones. A channel
outside the type's set never delivers and users cannot enable it in their preferences (the cell
shows as locked).
...
Registering a new channel does not automatically make existing built-in types eligible for it.
Channel authors should declare it on notification types their module owns; operators can add it to
other types through the tenant-scoped override.
...
3. **Make sure the type is push-eligible** — business and actionable built-ins ship without push,
   so enable it for the type first (Settings → Notification Delivery, or
   `PATCH /api/notifications/types`). The `demo.silent_ping` and `demo.push_playground` fixtures
   already ship with push eligibility. The recipient must also not have opted out of push for that
   type.
