...
| `POST /api/auth/reset/validate` | IP | 10 | 300s | &mdash; |
...
The reset-confirm and reset-validate endpoints use IP-only limiting because no email is available at that point — both are reached with a token alone. Reset-validate is the read-only check the reset page makes on load to decide whether to render the password form or a terminal "link is no longer valid" state; it never consumes the token, so it gets a slightly larger budget than reset-confirm.
...
| `RATE_LIMIT_RESET_VALIDATE_POINTS` | `10` | Max reset-token checks per IP per window |
| `RATE_LIMIT_RESET_VALIDATE_DURATION` | `300` | Window in seconds |
