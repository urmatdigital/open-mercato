...
Beyond core and app-level modules, the community publishes reusable modules to the **[Official Modules repository](https://github.com/open-mercato/official-modules)**. These are reviewed, installable in one command, and ejectable for full local ownership. If you've built a module that others could use, this is the recommended way to share it without modifying the core platform.

See [Official Modules](./official-modules) for installation, ejection, and contribution details.

## Learning from a Real Module

Before building from scratch, check the **[Reference Example Module](./reference-example-module)** showcase — a capability-by-capability index into `apps/mercato/src/modules/example/`, the canonical, source-present-but-runtime-disabled module that demonstrates nearly every extension point this framework exposes (entities, APIs, forms, events, UMES, AI tools, and more), each with an exact link to the file that proves it.
