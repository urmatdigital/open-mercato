...
Three message providers ship today, all `channelScope: 'user'`:
...
- `@open-mercato/channel-discord` — Discord bot: REST outbound, real-time inbound over a
  provider-owned Gateway (WebSocket) worker, and an Ed25519-signed Interactions endpoint. The first
  non-email provider, and the first with `capabilities.recipientFormat: 'provider-native'` and
  `realtimePush: true`. See [Set up Discord](../../user-guide/communication-channels-discord.mdx)
...
- **Providers**: `packages/channel-{imap,gmail}/` (email), `packages/channel-discord/` (chat) and `packages/channel-{fcm,apns,expo}/` (push)
...
[Building on the Communications Hub](building-communication-channel-provider.mdx) covers this
end-to-end — the adapter contract, the capability rules, fail-closed webhook verification, the
worker pattern for transports that are not webhooks, and tenant-scoped provider state. The steps
below are the short version.
:::

The hub deliberately stays adapter-agnostic. To add another email provider (e.g. Yahoo, ProtonMail Bridge variant), follow the same shape as the shipping packages:
...
`capabilities.recipientFormat` tells the hub what an outbound recipient looks like for this provider:

| Value | Hub validation | Use for |
|---|---|---|
| `'email'` (default when omitted) | Full email-address validation | Email providers — Gmail, IMAP |
| `'provider-native'` | Length plus transport safety only — the recipient must match the allowlist `^[A-Za-z0-9._:@+-]+$` and must not contain `..` | Providers addressed by a provider-issued id — e.g. a Discord channel snowflake |

A provider that declares `'provider-native'` owns the format check itself and MUST treat the recipient as untrusted input — the hub deliberately does not know what a valid id looks like for your transport.

The transport-safety check is an **allowlist**, not a denylist, because the recipient reaches adapters that interpolate it into a REST path (Discord posts to `/channels/{recipient}/messages`). Anything outside the permitted set — path separators, query and fragment markers, whitespace, percent-encoded separators such as `%2F`, and control bytes — is rejected before your adapter sees it. If your transport's identifiers need a character outside that set, widen the allowlist in `lib/outbound-recipient.ts` deliberately rather than routing around the check.
...
See the shipping providers for end-to-end examples:
...
- Discord: `packages/channel-discord/src/modules/channel_discord/lib/adapter.ts` — the non-email
  reference: `provider-native` recipients, `realtimePush`, a provider-owned Gateway worker instead
  of a webhook, and a provider-owned signed route for the interactions handshake

## Related

- [Building on the Communications Hub](building-communication-channel-provider.mdx) — consume the hub from your own module, or ship a new provider
...
- [User guide: Communication Channels](../../user-guide/communication-channels.mdx) · [Set up Discord](../../user-guide/communication-channels-discord.mdx)
