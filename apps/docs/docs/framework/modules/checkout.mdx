...
Checkout uses the Communications Hub system email transport. Omit `SYSTEM_EMAIL_PROVIDER` or set it to `resend` for the Resend channel provider, or set `SYSTEM_EMAIL_PROVIDER=ses` for the Amazon SES channel provider.
