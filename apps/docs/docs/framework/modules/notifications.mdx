...
Notification emails use the Communications Hub system email transport. Omit `SYSTEM_EMAIL_PROVIDER` or set it to `resend` for the Resend channel provider, or set `SYSTEM_EMAIL_PROVIDER=ses` for the Amazon SES channel provider. SES uses the AWS SDK credential chain and `AWS_SES_REGION` / `AWS_REGION` when present.
