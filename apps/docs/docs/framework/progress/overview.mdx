...
| `touchJobHeartbeat(jobId, ctx)` | Optional. Bump `heartbeatAt` only, on a forked EntityManager — keeps a job alive across a single unit of work longer than the stale timeout. Always optional-chain it |
...
:::caution One unit of work longer than the timeout
The sweep cannot tell "slow" from "dead" — a producer that only heartbeats between batches gets failed whenever a single batch runs past the timeout. If one unit of work can exceed 60s (a slow upstream API page, a large reindex step), keep the job alive **while you await it**:

```typescript
const timer = setInterval(() => {
  void progressService.touchJobHeartbeat?.(jobId, ctx)
}, 15_000)
try {
  await slowUpstreamCall()
} finally {
  clearInterval(timer)
}
```

Bind the timer to the pending await, never to the operation as a whole — a heartbeat that outlives its producer keeps a dead job green forever. `withHeartbeat` in `data_sync/lib/sync-engine.ts` is the reference wrapper for async iterators.

Should a job still be swept while alive, the next progress write revives it: the write paths retry once through the start transition, restricted to rows the sweep marked so a genuine failure keeps its error message. `completed` and `cancelled` are never revived.
:::
