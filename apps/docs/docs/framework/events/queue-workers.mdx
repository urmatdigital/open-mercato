...
Lazy mode replaces the eager worker with a lightweight watcher process. The watcher probes each discovered queue for ready jobs without importing handler code or creating BullMQ workers. By default, the first ready job on a queue triggers `mercato queue worker <queueName>`; queues that never receive jobs stay completely idle.

For memory-sensitive local development, set `OM_AUTO_SPAWN_WORKERS_LAZY_MODE=shared`. In that mode the watcher still stays idle until the first ready job appears, but it starts one shared `mercato queue worker --all` process instead of one process per active queue. This keeps queue behavior intact while avoiding several full CLI bootstraps when scheduler-driven jobs make many queues ready at once.

```bash
# Enable lazy auto-spawn when bypassing the dev wrapper
OM_AUTO_SPAWN_WORKERS_LAZY=true mercato server dev
```

The monorepo and standalone `yarn dev` wrappers set `OM_AUTO_SPAWN_WORKERS_LAZY=true`, `OM_AUTO_SPAWN_WORKERS_LAZY_MODE=shared`, and `OM_AUTO_SPAWN_SCHEDULER_LAZY=true` automatically for local memory reduction. Direct `mercato server dev` and production `mercato server start` keep the eager default until these variables are set explicitly. Use `OM_AUTO_SPAWN_WORKERS_LAZY=false OM_AUTO_SPAWN_SCHEDULER_LAZY=false yarn dev` to force the historical eager mode.
...
| `OM_AUTO_SPAWN_WORKERS_LAZY_MODE` | `per-queue` | `per-queue` starts one worker process per queue after that queue receives a job. `shared` starts one `queue worker --all` process after the first ready job. |
...
| `OM_AUTO_SPAWN_SCHEDULER_LAZY` | `false` | Enables lazy local scheduler startup. Ignored when the scheduler is disabled by `AUTO_SPAWN_SCHEDULER=false`. |
| `OM_AUTO_SPAWN_SCHEDULER_LAZY_POLL_MS` | `1000` | Enabled-schedule probe interval. Clamped to a minimum of `250`. |
| `OM_AUTO_SPAWN_SCHEDULER_LAZY_RESTART` | `true` | Restart the scheduler process if it exits unexpectedly while enabled schedules remain. |
...
- Lazy scheduler mode does not change schedule execution. It still launches the existing `mercato scheduler start` process after an enabled schedule exists.
...
:::warning A worker must actually be running
With single-delivery on (the default), persistent subscribers run **only** in the events worker. The
bus does not check whether one exists - the queue is durable, so an emit with no worker yet is
delayed, not lost, and a worker started later drains the backlog. Make sure something drains the
`events` queue.

`mercato server`/`mercato server start` reconciles this for a process it *knows* runs no worker: with
`AUTO_SPAWN_WORKERS=false` and no `OM_EVENTS_EXTERNAL_WORKER`, that bootstrap rewrites
`OM_EVENTS_SINGLE_DELIVERY=false` so subscribers run inline instead. A custom entrypoint (a bare
`next start`, your own server) never runs that bootstrap, so nothing is rewritten for it - run a
worker.

The worker dispatches through the DI event bus, so it must run inside a bootstrapped process
(`mercato queue worker ...`, or any entrypoint that builds a request container). A worker that cannot
resolve `eventBus` fails the job loudly rather than completing it with nothing dispatched.
:::
