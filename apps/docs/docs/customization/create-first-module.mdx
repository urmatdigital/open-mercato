...
Browse `apps/mercato/src/modules/example/` for a full working module that exercises most platform features (entities, APIs, forms, grids, events, dashboard widgets, custom fields, and injection) — or start from the [Reference Example Module](../framework/modules/reference-example-module) showcase, which indexes every capability it demonstrates with an exact link to the source. Core modules from `@open-mercato/core` can be customized by adding override files under `apps/mercato/src/modules/auth`, `apps/mercato/src/modules/directory`, etc.
:::
