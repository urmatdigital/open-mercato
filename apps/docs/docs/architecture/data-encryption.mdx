...
That detection runs **only while no DEK is reachable** — `disabled` or `unavailable` below, the
states where shape is the only test available. A shape is forgeable, and in `active` mode the
indexer really did decrypt the document, so a value that still looks like an envelope there is
plaintext somebody typed and is indexed as such.
