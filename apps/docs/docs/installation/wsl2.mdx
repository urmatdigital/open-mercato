...
- At least 16 GB of RAM recommended (8 GB minimum for the infra-only compose with the app running natively; the full containerized stack needs 12 GB minimum)
...
If you are on Windows 10 or an older WSL build, omit that line and use the [Windows host IP approach](#9-connecting-wsl2-to-a-windows-hosted-database) described later.
:::
...
Verify: `docker compose --project-directory . -f starters/docker/compose.infra.yml ps` — all three services (postgres, redis, meilisearch) should be `Up`.
...
On WSL2, raising the Linux inotify limits enables the preferred Turbopack dev server:
...
| `OS file watch limit reached` from Turbopack | Run `yarn dev:fix-wsl-watchers` for the preferred Turbopack path; in restricted containers/sandboxes, use the automatic Webpack fallback or set `OM_DEV_BUNDLER=webpack`. |
