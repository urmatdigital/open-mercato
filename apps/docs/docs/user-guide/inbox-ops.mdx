...
:::info Resend-specific receiving flow
The Communications Hub system email transport can use Resend or Amazon SES for outbound transactional email, but Inbox Ops receiving still uses Resend webhooks to fetch inbound email content. Setting `SYSTEM_EMAIL_PROVIDER=ses` for outbound transactional emails does not replace the Resend webhook setup described here.
:::
