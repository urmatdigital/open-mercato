...
- **WAIT_FOR_CONDITION**: Pauses until something becomes true about the run's own data
- **IF_ELSE** / **SWITCH**: Decision steps that route on a condition or on one field's value

[**Learn more about step types →**](./step-types)

### Transitions

Transitions — "routes" on the canvas — connect steps and define how the workflow moves forward. They can be automatic (immediate) or conditional (based on rules or manual actions), and a route marked as an **error route** is taken only when its source step fails.
